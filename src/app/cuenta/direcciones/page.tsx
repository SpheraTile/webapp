...
export default function DireccionesPage() {
  ...
  const tienesDireccion = formData.direccion && formData.ciudad && formData.codigo_postal

  return (
    <div className="min-h-screen bg-white lg:bg-neutral-50">
      <DesktopNav />
      <div className="lg:hidden">
        <Header titulo="Direcciones de Envío" showBack />
      </div>
      ...
      <div className="lg:pt-20 lg:max-w-2xl lg:mx-auto lg:px-6 lg:py-12">
        <div className="hidden lg:flex lg:items-center lg:justify-between lg:mb-8">
          <div>
            <h1 className="text-3xl font-bold text-neutral-900">Direcciones de Envío</h1>
            <p className="text-neutral-500 mt-1">Gestiona tu dirección de entrega</p>
          </div>
          <Link
            href="/cuenta"
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-primary-900">Dirección principal</span>
                    <span className="px-2 py-0.5 bg-primary-200 text-primary-800 text-xs rounded-full">Por defecto</span>
                  </div>
                  <p className="text-primary-800 mt-1">{formData.nombre}</p>
                  <p className="text-primary-700 text-sm">{formData.direccion}</p>
                  <p className="text-primary-700 text-sm">
                    {formData.codigo_postal} {formData.ciudad}
                    {formData.provincia && `, ${formData.provincia}`}
                  </p>
                  {formData.telefono && (
                    <p className="text-primary-600 text-sm mt-1">Tel: {formData.telefono}</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit} className="lg:bg-white lg:rounded-2xl lg:shadow-sm">
          <div className="p-4 lg:p-8 border-b border-neutral-100">
            <h2 className="font-semibold text-neutral-900">
              {tienesDireccion ? 'Editar dirección' : 'Añadir dirección de envío'}
            </h2>
          </div>
          ...
          <div className="p-4 lg:p-8 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Nombre para la entrega"
              />
            </div>
